...
//using MSXML2;     //VS2005 添加引用-COM-Microsoft XML, V2.6
...
namespace Daan.Interface.Util
{
    ...
    public class XMLHelper : IDisposable
    {
        ...
        /// <returns></returns>
        public static DataBaseinfo ReadDataBase(string databasename)
        {
            DataBaseinfo baseinfo = new DataBaseinfo();

            #region zhouy 读取数据库配置文件

            XmlDocument XD = new XmlDocument();
            XD.Load(databasename);
            ...
            for (int j = 0; j < xn.ChildNodes.Count; j++)
            {
                ...
                {
                    case "databasetype":
                        baseinfo.databasetype = clxn.InnerText;
                        break;
                    case "userid":
                        baseinfo.userid = clxn.InnerText;
                        break;
                    case "password":
                        baseinfo.password = clxn.InnerText;
                        break;
                    case "dataname":
                        baseinfo.dataname = clxn.InnerText;
                        break;
                    case "datasource":
                        baseinfo.datasource = clxn.InnerText;
                        break;
                    case "port":
                        baseinfo.port = clxn.InnerText;
                        break;
                    default:
                        break;
                }
